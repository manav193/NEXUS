...
test("approval lifecycle is PENDING -> APPROVED -> CONSUMED", () => {
  ...
  const request = createApprovalRequest({ action, decision: { decision: Decision.REQUIRE_APPROVAL }, requester: "agent", ttlMs: 60000, createdAt: new Date(now).toISOString(), policyVersion:"p1" });
  ...
  const consumed = store.consume(request.id, { actor: "agent", action, policyVersion:"p1" });
  assert.equal(consumed.approved, true);
  assert.equal(consumed.request.status, ApprovalStatus.CONSUMED);
  assert.equal(store.consume(request.id, { action, policyVersion:"p1" }).approved, false);
});
...
test("expired approval cannot be consumed", () => {
  ...
  now += 1001;
  const result = store.consume(request.id, { action });
...
test("gateway executes only with a valid one-time approval", async () => {
  ...
  const request = createApprovalRequest({ action, decision, requester: "agent", policyVersion:"p1" });
  store.create(request);
  store.decide(request.id, { approved: true, approver: "human" });

  const result = await executeThroughGateway({ action, adapter, approvalStore: store, approvalRequestId: request.id, auditSink: audit, policyVersion:"p1" });
  assert.equal(result.executed, true);
  assert.equal(adapter.calls.length, 1);

  const replay = await executeThroughGateway({ action, adapter, approvalStore: store, approvalRequestId: request.id, auditSink: audit, policyVersion:"p1" });
...
test("policy version mismatch cannot consume approval", () => {
  const store = createMemoryApprovalStore();
  const action = createAction({ actor: "agent", tool: "shell", operation: "execute_shell" });
  const request = createApprovalRequest({ action, decision: { decision: Decision.REQUIRE_APPROVAL }, requester: "agent", policyVersion:"p1" });
  store.create(request);
  store.decide(request.id, { approved: true, approver: "human" });
  const result = store.consume(request.id, { action, policyVersion:"p2" });
  assert.equal(result.approved, false);
  assert.equal(result.reason, "APPROVAL_POLICY_VERSION_MISMATCH");
});
