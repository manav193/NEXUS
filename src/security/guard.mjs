...
import { fingerprintAction as canonicalFingerprint } from "../core/canonical.mjs";

export const SecurityReason = Object.freeze({
...
const INJECTION_PATTERNS = [
  ...
  /system\s+prompt/i,
  /reveal\s+(the\s+)?(system|developer)\s+instructions/i,
  /bypass\s+(security|policy|approval)/i,
  /disable\s+(safety|security|policy)/i,
];
...
export function createSecurityGuard({ allowedTools = [], allowedOperations = {}, killSwitch = null } = {}) {
  ...
  return Object.freeze({
    inspect(action) {
      ...
      const allowed = operations.get(action.tool);
      if (allowed && !allowed.has(action.operation)) reasons.push(SecurityReason.OPERATION_NOT_ALLOWED);
      ...
      return Object.freeze({
        ...
        decision: reasons.length ? Decision.DENY : Decision.ALLOW,
        reasonCodes: Object.freeze(reasons),
      });
    },
  });
}
export function createKillSwitch({ active = false } = {}) {
  let state = Boolean(active);
  return Object.freeze({ activate(){state=true;}, deactivate(){state=false;}, isActive(){return state;} });
}
export function redactSecrets(value) {
  if (typeof value === "string") return value
    .replace(/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----[\s\S]*?-----END (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/g,"[REDACTED_PRIVATE_KEY]")
    .replace(/(?:api[_-]?key|access[_-]?token|secret|password)\s*[:=]\s*(['"]?)[A-Za-z0-9_\-./+=]{12,}\1/gi,"$1[REDACTED_SECRET]$1")
...
export { canonicalFingerprint as fingerprintAction };
